...
class GenericClient(CommonConfig):
    ...
    # pylint: disable=no-self-use
    def _mkrichtext(self, text, tags, maxlen=None, etc="...", separator=" |"):
        """
        ...
        for word in inline_kw:
            pattern = r"%s(%s)%s" % (before_tag, re.escape("%s" % word), after_tag)
        ...
        if maxlen is not None:
            assert not len(to_return) > maxlen, "{}:{} : {} > {}".format(
                text, to_return, len(to_return), maxlen
            )
    ...
    def download_media(self, the_url):
        ...
        def get_filename_from_response(the_response):
            ...
            # Sanity check
            if not re.match(r"^[\w-]+\.(jpg|jpeg|gif|png)$", to_return, re.IGNORECASE):
                # Nope, "bad" filename
                logging.error("Invalid media filename '%s' - ignoring", to_return)
                to_return = ""
        ...
        filename = (
            get_filename_from_cd(request.get_header("Content-Disposition"))
            or get_filename_from_response(response)
            or "random.jpg"
        )
    ...
    def post_within_limits(self, entry_to_post, feed):
        ...
        post_from_feed = (
            not feed.is_post_limited() or feed.posts_done < feed.config["max_posts"]
        )
        post_to_client = (
            not self.is_post_limited() or self.posts_done < self.config["max_posts"]
        )
    ...
    def seeding_published_db(self, entry_count, feed, feed_count):
        ...
        # consideration independently; don't use resolve_option
        seed_client = (
            self.config["max_posts"] < 0 and entry_count + self.config["max_posts"] <= 0
        )
        seed_feed = (
            feed.config["max_posts"] < 0 and feed_count + feed.config["max_posts"] <= 0
        )
        ...
        if the_url and url_shortener and url_shortener != "none":
            try:
                ...
                if len(to_return) > len(the_url):
                    ...
                    raise RuntimeError(
                        "Shortener %s produced a longer URL " + "than the original!",
                        url_shortener,
                    )
            # pylint: disable=broad-except
                ...
                if isinstance(exception, AssertionError):
                    ...
                    logging.error("URL shortener %s is unimplemented!", url_shortener)
                    logging.info("Available URL shorteners: %s", all_shorteners)
                else:
                    logging.error(
                        "Cannot shorten URL %s with %s: %s",
                        the_url,
                        url_shortener,
                        str(exception),
                    )
    ...
    def filter_tags(self, feed, entry):
        ...
        if (not (tag_filter_opts and "ignore_title" in tag_filter_opts)) and entry.tags[
            "title"
        ]:
        ...
        ) and entry.tags["content"]:
            to_filter.extend(entry.tags["content"])
        # Finally, category tags, again if appropriate
        if (
            not (tag_filter_opts and "ignore_category" in tag_filter_opts)
        ) and entry.tags["category"]:
        ...
        for tag in to_filter:
            if (
                tag_filter_opts
                and "case-sensitive" in tag_filter_opts
                and tag not in to_return
            ):
                to_return.append(tag)
            elif (
                not (tag_filter_opts and "case-sensitive" in tag_filter_opts)
            ) and tag.lower() not in non_case_sensitive:
                to_return.append(tag)
    ...
    def remove_ending_tags(self, feed, content):
        ...
        tag_filter_opts = self.resolve_option(feed, "tag_filter_opts")
        if content and (not (tag_filter_opts and "ignore_content" in tag_filter_opts)):
            tag_pattern = r"\s+#([\w]+)$"
    ...
    def strip_html(self, feed, before_strip):
        ...
        while not done:
            to_return = lxml.html.fromstring(before_strip).text_content().strip()
