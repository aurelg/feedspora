...
class FacebookClient(GenericClient):
    ...
    def post(self, feed, entry):
        ...
        text = ""
        if (
            self.resolve_option(feed, "post_include_content")
            and stripped_html
            or not self.resolve_option(feed, "post_include_media")
        ):
        ...
        text += "".join([" #{}".format(k) for k in self.filter_tags(feed, entry)])
        if not self.resolve_option(feed, "post_include_media"):
        ...
        if self.is_testing():
            ...
            to_return = self._graph.put_object(
                self._config["post_to_id"], "feed", **attachment
            )
