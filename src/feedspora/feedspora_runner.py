...
class FeedSpora:
    ...
    def is_already_published(self, entry, client):
        ...
        sql = (
            "SELECT id from posts WHERE feedspora_id=:feedspora_id AND "
            "client_id=:client_id"
        )
        self._cur.execute(
            sql, {"feedspora_id": pub_item, "client_id": client.config["name"]}
        )
        already_published = self._cur.fetchone() is not None

        if already_published:
            logging.info(
                "Skipping already published entry in %s: %s",
                client.config["name"],
                entry.title,
            )
        else:
            logging.info(
                "Found entry to publish in %s: %s", client.config["name"], entry.title
            )
    ...
    def add_to_published_entries(self, entry, client):
        ...
        self._cur.execute(
            "INSERT INTO posts (feedspora_id, client_id) " "values (?,?)",
            (pub_item, client.config["name"]),
        )
        self._conn.commit()
    ...
    def _publish_entry(self, entry, entry_count, feed, feed_count):
        ...
        if not self._client:
            logging.error("No client found, aborting publication", exc_info=True)
        ...
        for client in self._client:
            if not self.is_already_published(entry, client):
                ...
                try:
                    ...
                    logging.error(
                        "Error while publishing '%s' to client" " '%s' : %s",
                        entry.title,
                ...
                if posted_to_client or client.seeding_published_db(
                    entry_count, feed, feed_count
                ):
                    try:
                        self.add_to_published_entries(entry, client)
                    except Exception as error:
                        logging.error(
                            "Error while storing '%s' to client" "'%s' : %s",
                            entry.title,
                            client.__class__.__name__,
                            format(error),
                            exc_info=True,
        ...
        if entry_generator:
            ...
            for entry in entry_generator:
                ...
                if feed.max_posts_done():
                    ...
                    logging.info(
                        "Configured feed limit of %d reached.", feed.config["max_posts"]
                    )
