...
class GenericFeed(CommonConfig):
    ...
    def max_posts_done(self):
        ...
        return (
            ...
            and self._posts_done >= self._config["max_posts"]
        )
    ...
    def parse_atom(self, soup):
        ...
        for entry in soup.find_all("entry")[::-1]:
            ...
            try:
                fse.title = (
                    BeautifulSoup(entry.find("title").text, "html.parser")
                    .find("a")
                    .text
                )
    ...
    def find_rss_image_url(self, entry, link):
        ...
        if (
            entry.find("media:content")
            and entry.find("media:content")["medium"] == "image"
        ):
    ...
    def feed_generator(self):
        ...
        except (requests.exceptions.ConnectionError, ValueError, OSError) as error:
            logging.error(
