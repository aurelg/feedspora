...
class MastodonClient(GenericClient):
    ...
    def __init__(self, config, testing):
        ...
        self._visibility = (
            ...
            or config["visibility"] not in ["public", "unlisted", "private"]
    ...
    def post(self, feed, entry):
        ...
        maxlen = (
            ...
            - len(self.resolve_option(feed, "post_suffix"))
            - 1
        )
        ...
        stripped_html = self.strip_html(feed, entry.content) if entry.content else None
        if self.resolve_option(feed, "post_include_content") and stripped_html:
            raw_contents += ": " + stripped_html
        text += self._mkrichtext(
            raw_contents, self.filter_tags(feed, entry), maxlen=maxlen
        )
        ...
        if self.is_testing():
            ...
            if media_path:
                ...
                except (MastodonIllegalArgumentError, MastodonAPIError) as exception:
                    logging.info(
                        "Error encountered while posting %s: %s",
                        media_path,
                        str(exception),
                    )
