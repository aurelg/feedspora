...
class WPClient(GenericClient):
    ...
    def __init__(self, config, testing):
        ...
        if not testing:
            self.client = Client(
                config["wpurl"], config["username"], config["password"]
            )
    ...
    def get_content(self, url):
        ...
        if (
            ...
            and request.headers["Content-Type"].find("html") != -1
        ):
            try:
    ...
    def post(self, feed, entry):
        ...
        article_content = ""
        if "post_link_content" in self._config and self._config["post_link_content"]:
        ...
        if self.is_testing():
            ...
            if (
                "post_link_content" in self._config
                and self._config["post_link_content"]
            ):
                content = "From " + self.shorten_url(feed, entry.link)

            self.accumulate_testing_output(
                self.get_dict_output(
                    feed=feed, entry=entry, content=content, media_path=media_path
                )
            )
        else:
            ...
            post = WordPressPost()
            post.title = (
                self.resolve_option(feed, "post_prefix")
                + entry.title
