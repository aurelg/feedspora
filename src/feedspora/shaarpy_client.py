...
class ShaarpyClient(GenericClient):
    ...
    def __init__(self, config, testing):
        ...
        """
        self._config = config
        if "post_audience" in config and config["post_audience"].lower() == "private":
            self._post_private = True

        if not testing:
            self._shaarpy = Shaarpy()
            self._shaarpy.login(config["username"], config["password"], config["url"])
    ...
    def post(self, feed, entry):
        ...
        if self.is_testing():
            post_args = {
                ...
                "title": title,
                "content": content,
                "audience": "private" if self._post_private else "public",
            }
            ...
            try:
                to_return = self._shaarpy.post_link(
                    link, tags, title=title, desc=content, private=self._post_private
                )
